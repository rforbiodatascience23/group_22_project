...
The second script of analysis will be related to the performing PCA and the creation of some logistic regression models with different variations. These variations are based on the creation of three different models. The first of them will use the components used to reach 80% variability after the PCA analysis and the second and third ones will be different models for men and women. Thanks to this we will be able to analyze whether gender has a special importance when predicting patients with diabetes.
...
It can be noticed that the difference among having diabetes or not is being quite well distinguished in the two first components. We will continue by cretaing the rotation matrix. In PCA this matrix is like a set of instructions that turns our data points into a new set of directions. It helps to highlight the most important information in the data. We need to think of it as finding the best view tu understand patterns in the data.
...
To help us finding the desired number of components, we will create a scree plot and select those whose cumulative sum achives an 80% of cumulative variance.
...
```{r}
model <- glm(Diabetes_binary ~ ., family = "binomial", data = women_data)

predictions <- predict(model, type = "response")

binary_predictions <- ifelse(predictions > 0.5, 1, 0)

accuracy <- mean(binary_predictions == women_data$Diabetes_binary)
...
```{r}
model <- glm(Diabetes_binary ~ ., family = "binomial", data = men_data)

predictions <- predict(model, type = "response")

binary_predictions <- ifelse(predictions > 0.5, 1, 0)

accuracy <- mean(binary_predictions == men_data$Diabetes_binary)
