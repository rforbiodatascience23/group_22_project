...
```{r}
df <- read.table(file = "../data/02_dat_clean.tsv", 
                 sep = "\t", 
                 header = TRUE)
...
# Assuming your dataset is stored in a variable named 'df'
# Replace 'df' with the actual data frame name
# Assume 'Diabetes_binary' is the outcome variable (0 or 1)

# Load necessary libraries
library(stats)

# Numeric variables for t-tests
numeric_vars <- c("HighBP", "HighChol")  # Replace with actual numeric variables
...
numeric_results <- lapply(numeric_vars, function(var) {
  t_test_result <- t.test(diabetic_group[[var]], non_diabetic_group[[var]])
  return(data.frame(
    Variable = var,
    Mean_Diabetic = mean(diabetic_group[[var]]),
    Mean_NonDiabetic = mean(non_diabetic_group[[var]]),
